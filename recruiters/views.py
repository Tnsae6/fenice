...
def add_job(request):
    user = request.user
    verified = CompanyProfile.objects.filter(user=request.user).first()
...
def my_company_profile(request):
    you = request.user
    profile = CompanyProfile.objects.filter(user=you).first()
    context = {
        'u': you,
        'profile': profile,
        'rec_navbar': 1,
    ...
    return render(request, 'recruiters/company_profile.html', context)

@login_required
def profile_view(request, slug):
    p = CompanyProfile.objects.filter(slug=slug).first()
    you = p.user
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        form = CompanyProfileUpdateForm(request.POST, request.FILES, instance=profile)
        if form.is_valid():
            data = form.save(commit=False)
            data.user = you
            data.save()
            return redirect('my-company-profile')
    else:
    ...
    return render(request, 'recruiters/edit_Company_profile.html', context)
